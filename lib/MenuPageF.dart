...
class MenuPageF extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return MenuPageFState();
  }
}

class MenuPageFState extends State<MenuPageF> {
  ...
  Future choiceImage() async {
    ...
    final uri = Uri.parse("http://10.0.2.2/biopure_app/upload_fournisseur.php");
    var request = http.MultipartRequest('POST',uri);
    request.fields['id_fournisseur'] = id;
    ...
    image = data[0]["image"];
    Navigator.push(context, MaterialPageRoute(builder: (context) => MenuPageF()));
  }

  Future updatePWFournisseur(String motPasse) async {
    await http.post("http://10.0.2.2/biopure_app/update_pw_fournisseur.php", body: {
      "id_fournisseur": id,
      "mot_passe_fournisseur": motPasse
    });
  }
      ...
      child: Drawer(
        child: Column(
          ...
          children: <Widget>[
            ...
            SizedBox(height: 20),
            ListTile(
              leading: Icon(Icons.home_filled, size: height*0.04),
              title: Text("Accueil", style: TextStyle(fontSize: height*0.025)),
              onTap: () {
                print("Accueil");
            ...
            ListTile(
              leading: Icon(Icons.history, size: height*0.04),
              title: Text("Historique", style: TextStyle(fontSize: height*0.025)),
              onTap: () {
                print("Historique");
                Navigator.push(context, MaterialPageRoute(builder: (context) => HistoriqueCmdPage()));
              }
            ),
            SizedBox(height: height*0.02),
            ListTile(
          ...
          MaterialButton(
            ...
            onPressed: () {
              if (controllerPW.text.isNotEmpty) {
                updatePWFournisseur(controllerPW.text);
