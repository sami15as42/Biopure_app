...
class AddStatutPage extends StatefulWidget {
  @override
  _AddStatutPageState createState() => _AddStatutPageState();
}

class _AddStatutPageState extends State<AddStatutPage> {

  TextEditingController controllerStatut = new TextEditingController();

  Future addStatut() async {
    await http.post("http://10.0.2.2/biopure_app/add_statut.php", body: {
      "designation_statut": controllerStatut.text
    });
    setState(() {
      controllerStatut.text = "";
    });
  }
                    ...
                    child: ListView(
                      ...
                      children: <Widget>[
                        ...
                        Row(
                          children: <Widget>[
                            const Spacer(),
                            Text('Ajouter un statut', style: TextStyle(fontSize: height*0.03)),
                            const Spacer(),
                          ],
                        ),
                        Padding(
                              ...
                              children: <Widget>[
                                Row(children: <Widget>[
                                  Padding(
                                    ...
                                    child: Text('Désignation du statut', style: TextStyle(fontSize: height*0.02, color: Colors.grey[500])),),
                                ]),
                                Padding(
                                ...
                                CupertinoButton(child: Icon(Icons.add, color: Colors.white, size: height*0.03), padding: EdgeInsets.fromLTRB(8, 8, 8, 8), pressedOpacity: 0.7, borderRadius: BorderRadius.circular(50), color: Colors.blue, 
                                  onPressed: () {
                                    if (controllerStatut.text.isNotEmpty) addStatut();
                                  }
                                ),
                                SizedBox(height: height*0.02),
                              ],
